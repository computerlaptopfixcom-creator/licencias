...
export async function POST(request: Request) {
  return withAdmin(async (_, req) => {
    try {
      const { category, name, price, iconType } = await req.json();
      if (!category || !name || price === undefined) {
        return NextResponse.json({ error: 'Categoría, nombre y precio son requeridos' }, { status: 400 });
      }
      const item = addCatalogItem(category, name, parseFloat(price), iconType);
      if (!item) {
...
export async function PUT(request: Request) {
  return withAdmin(async (_, req) => {
    try {
      const { id, category, name, price, iconType } = await req.json();
      if (!id) {
        return NextResponse.json({ error: 'ID requerido' }, { status: 400 });
      }
      const result = updateCatalogItem(id, { category, name, price: price !== undefined ? parseFloat(price) : undefined, iconType });
      if (!result.success) {
