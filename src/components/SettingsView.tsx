...
import { Settings, ShieldCheck, ShieldAlert, Key } from 'lucide-react';
        ...
        {user.role === 'admin' && (
          <div className="space-y-6">
            <div className="flex items-center gap-3 text-blue-500">
              ...
              <h3 className="text-2xl font-black uppercase tracking-tighter">Branding y Personalizacion</h3>
            </div>
            <form onSubmit={saveSettings} className="bg-[#111] border border-white/10 rounded-[2.5rem] p-8 shadow-2xl grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-2 md:col-span-2">
                <label className="text-[10px] font-black uppercase text-white/40 tracking-widest pl-1">Descripcion de la App</label>
        ...
        {user.role === 'admin' && (
          <div className="space-y-6">
            <div className="flex items-center gap-3 text-blue-400">
              ...
              <h3 className="text-2xl font-black uppercase tracking-tighter">Integracion Telegram Bot</h3>
            </div>
            <div className="bg-[#111] border border-white/10 rounded-[2.5rem] p-8 shadow-2xl space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="md:col-span-2 space-y-2">
                  <label className="text-[10px] font-black uppercase text-white/40 tracking-widest pl-1">URL completa del Webhook (opcional)</label>
                  <input value={settings.telegramWebhookUrl} onChange={(e) => setSettings({ ...settings, telegramWebhookUrl: e.target.value })} placeholder="https://tu-app.vercel.app/api/telegram/webhook" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 text-sm font-mono focus:border-blue-500 outline-none transition-colors" />
                  <p className="text-[10px] text-white/20 italic pl-1">Si lo dejas vacio, el sistema intentara generarlo automaticamente con el dominio actual.</p>
                </div>
              </div>
          ...
          <div className="bg-[#111] border border-white/10 rounded-[2.5rem] p-8 shadow-2xl space-y-6">
            ...
            {!showPassForm ? (
              ...
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  if (currentPass.length >= 1 && newPass.length >= 6) {
              ...
              >
                <div className="flex justify-between items-center mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-lg font-black uppercase tracking-tighter">Actualizar Contrasena</h4>
                  </div>
                ...
                <div className="space-y-4">
                  <input
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-sm focus:border-blue-500 outline-none transition-colors"
                  />
            ...
            <div className="p-6 bg-red-500/5 border border-red-500/10 rounded-3xl flex items-center justify-between group cursor-pointer hover:bg-red-500/10 transition-all">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-bold text-red-500/40">La eliminacion de la cuenta es irreversible</span>
                </div>
              </div>
